...
An AWS Lambda function written in Go that automates the application of AWS ElastiCache service updates across clusters and replication groups. It reads the mandatory `Environment` tag from each ElastiCache resource and enforces progressive bake-in periods across non-production and production promotion tiers.
...
| **Production** | **Prod** | `prod` / `production` | **7 days** | Applied once update is ≥ 7 days old |
...
- `AutoUpdatePolicy`: Optional tag (`disabled` to opt out of automated updates)
